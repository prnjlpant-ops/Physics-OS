...
export default function MemoryCard({ card, isBookmarked, onToggleBookmark, showChapter = false }) {
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-[#3c3c3c] bg-[#252526] p-3.5 transition-colors duration-150 hover:border-[#4a4a4a] print:break-inside-avoid print:border-[#c8c8c8] print:bg-white print:shadow-none">
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm font-medium leading-snug text-[#e8e8e8] print:text-black">
            {card.concept}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-2.5 border-t border-[#3c3c3c] pt-2.5 sm:grid-cols-2 print:border-[#c8c8c8]">
        <Field label="Short Explanation" value={card.shortExplanation} />
        <Field label="Why it Matters" value={card.whyItMatters} />
        <Field label="Typical Mistake" value={card.typicalMistake} />
      </div>
