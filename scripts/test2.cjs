...
// Pre-build a map from Roadmap Topic / Phase to pyq_index.json ID
const topicIdMap = {};

// We know the pattern: Topic 1 -> phase-a-01, Bonus #1 -> phase-a-bonus-01
for (let i = 1; i <= 20; i++) {
...
sheetNames.forEach(sheet => {
  ...
  data.forEach(row => {
    ...
    if (!tId && phaseStr !== 'Supplemental' && phaseStr !== 'Supplemental (feeds Topic 6/7)') {
       unmapped.add(phaseStr);
    }
  });
});

console.log('Unmapped:', Array.from(unmapped));
