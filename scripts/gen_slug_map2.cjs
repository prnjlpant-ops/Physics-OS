...
sheetNames.forEach(sheet => {
  ...
  data.forEach(row => {
    ...
    if (!tId && phaseStr && phaseStr.startsWith('Phase B')) {
      tId = getPhaseBTopicId(phaseStr, chapterName, subTopic);
    }
...
fs.writeFileSync('./src/engine/chapterToTopicMap.json', JSON.stringify(map, null, 2), 'utf8');
console.log('Wrote to chapterToTopicMap.json');
